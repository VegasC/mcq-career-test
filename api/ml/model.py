...
def index():
    ...
    correct_answer = np.asarray(pd.DataFrame(pd.read_csv(
        os.path.join(csv_dir, 'new_mcq_data.csv'), encoding='utf-8', header=None)))
    ...
    if len(answered_correctly) > 0:
        ...
        for correct_answer_cluster_prediction in correct_answer_cluster_predictions:
            if correct_answer_cluster_prediction == 2:
                correct_answer_cluster_predictions_department.append(
                    'computer')
        ...
        response = {
            "amount_of_questions_attend": len(request.get_json()['questions']),
            "amount_of_questions_answered_correctly": 0,
            "precentage_of_answered_questions_(%)": 0,
            "answered_correctly_departments": 0,
            "mcq_career_test_prediction": 0
        }
        return jsonify({'response': response})
